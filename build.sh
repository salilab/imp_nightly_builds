...
END
  ...
  # Normal full build
    ...
    if [ $PLATFORM = "mac10v10-intel" ] || [ $PLATFORM = "mac10v15-intel" ] || [ $PLATFORM = "mac11v0-intel" ] || [ $PLATFORM = "mac27arm64-gnu" ] || [ $PLATFORM = "mac14-intel" ]; then
      export LANG="en_US.UTF-8"
      get_cmake $PLATFORM
      CMAKE_ARGS+=("-DCMAKE_BUILD_TYPE=Release" \
                   "-DIMP_TIMEOUT_FACTOR=2" \
		   "-DCMAKE_CXX_FLAGS='-std=c++17'" \
                   ...
                   "-GNinja" \
                   "-DIMP_MAX_CHECKS=INTERNAL")
      if [ $PLATFORM = "mac27arm64-gnu" ]; then
