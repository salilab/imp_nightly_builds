...
def main():
    ...
    for m in ('mpi', 'spb', 'nestor'):
        mods = [release8, debug8, rh8_64, rh9_64, f40_64, fast8,
                coverage, win32, win64, mac12, mac12arm, fastmac, focal,
                jammy, noble]
    ...
    c.include_component('DOC', mac12arm)
    c.include_component('RMF-DOC', mac12arm)
    ...
    if imp_lab_check:
        ...
        for m in ('domino3',):
            c.exclude_component(m, (win32, win64, mac12arm, fastmac))
        if opts.imp_branch != 'develop':
            for m in ('pynet',):
        ...
        for m in ('isd_emxl',):
            incs = [release8, debug8, f40_64, fast8, coverage, win32,
                    win64, mac12, mac12arm, fastmac] + rh_rpms
            if opts.imp_branch != 'develop':
                incs.append(mac64)
